#!/usr/bin/env python3
"""Regenerate top 10 video pages with audio support."""
import sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Re-use gen_all_pages logic but only for top 10
import json, re

BASE = "/mnt/e/My_Projects/bztravel-english-study"
DATA_FILE = f"{BASE}/data/videos.json"
EXISTING_PAGES = {"xW5SlCvx2W0"}  # Iguazu already done
...
# Sort by views, skip Iguazu, take top 10
tops = sorted([v for v in videos if v["video_id"] not in EXISTING_PAGES],
...
def gen_quiz_js(quiz):
    ...
    for q in quiz:
        ...
        item = {"q": qtext, "choices": choices, "answer": answer, "explain": explanation}
...
function toggleZh(btn){var zh=btn.nextElementSibling;if(zh.style.display==='block'){zh.style.display='none';btn.innerHTML='📖 \\u663e\\u793a\\u4e2d\\u6587\\u7ffb\\u8bd1';}else{zh.style.display='block';btn.innerHTML='📕 \\u9690\\u85cf\\u4e2d\\u6587\\u7ffb\\u8bd1';}}'''
    ...
    fname = f"{slug}.html"
    path = f"{BASE}/video/{fname}"
    with open(path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"✅ {fname} with audio")

print("\n=== Top 10 pages regenerated with audio ===")
